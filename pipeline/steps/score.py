...
def run(ctx: RunContext, refine_out: RefineOutput) -> ScoreOutput:
    ...
    Quality scores are already computed in the refine step.  This step:
    1. Prunes tracks where the median frame-entry quality is below 0.0
       (the ``_prune_empty_workspace_tracks`` threshold is 0.0 because
       hard novelty pruning is disabled in v4.1; this step preserves
       the gate for future use).
    2. Marks each refined_track with ``pruned=True/False``.
    ...
    for track_dict in refine_out.refined_tracks:
        ...
        # Compute median quad-novelty for pruning
        novelty_scores = [
        ...
        # v4.1: hard pruning threshold is 0.0 (disabled); keep the gate for future tuning
    ...
    print(f"[Stage: Score] | {len(scored_tracks)} tracks scored | {len(pruned_instance_ids)} pruned | {active_count} active")
