...
def test_import_handler_output_persists_stage_events_and_resource_samples(tmp_path):
    db = _make_db(tmp_path)
    importer = ResultImporter(db_path=db, output_base=tmp_path)

    importer.import_handler_output(
        {
            "status": "complete",
    ...
    with sqlite3.connect(db) as conn:
        telemetry = json.loads(conn.execute(
            "SELECT detect_telemetry_json FROM pipeline_runs WHERE run_id='run-handler'"
        ).fetchone()[0])
        event = conn.execute(
