...
def test_large_jump_triggers_split():
    ...
    # First bbox: centroid at x=60 (center of [10,10,110,110])
    r1 = det.update(np.array([10, 10, 110, 110], dtype=np.float32), frame_width=1000)
    assert r1 is False  # first frame never triggers
    # Second bbox: centroid at x=550 (center of [500,10,600,110]) — jump of 490px > 300px threshold
    r2 = det.update(np.array([500, 10, 600, 110], dtype=np.float32), frame_width=1000)
    assert r2 is True
...
def test_gradual_drift_does_not_trigger():
    ...
    for i in range(10):
        x = i * 10
        result = det.update(np.array([x, 10, x+100, 110], dtype=np.float32), frame_width=1000)
...
def test_reset_clears_history():
    ...
    det = CentroidJumpDetector(jump_ratio=0.30, jump_within_frames=3)
    det.update(np.array([10, 10, 110, 110], dtype=np.float32), frame_width=1000)
    det.update(np.array([500, 10, 600, 110], dtype=np.float32), frame_width=1000)
    det.reset()
    # After reset, new updates should not be influenced by old history
    r = det.update(np.array([10, 10, 110, 110], dtype=np.float32), frame_width=1000)
    assert r is False
...
def test_first_frame_never_triggers():
    ...
    r = det.update(np.array([0, 0, 1000, 1000], dtype=np.float32), frame_width=1000)
    assert r is False
...
def test_uses_highest_scoring_centroid_when_multiple_candidates():
    ...
    r = det.update(np.array([500, 10, 600, 110], dtype=np.float32), frame_width=1000)
    assert r is True
